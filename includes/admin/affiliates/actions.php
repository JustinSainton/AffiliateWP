...
function affwp_process_add_affiliate( $data ) {
	...
	if( affwp_add_affiliate( $data ) ) {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=affiliate-wp-affiliates&affwp_notice=affiliate_added_failed' ) ); exit;
...
function affwp_process_update_affiliate( $data ) {
	...
	if( affwp_update_affiliate( $data ) ) {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=affiliate-wp-affiliates&action=edit_affiliate&affwp_notice=affiliate_updated&affiliate_id=' . $data['affiliate_id'] ) ); exit;

	} else {

		wp_safe_redirect( admin_url( 'admin.php?page=affiliate-wp-affiliates&affwp_notice=affiliate_update_failed' ) ); exit;
